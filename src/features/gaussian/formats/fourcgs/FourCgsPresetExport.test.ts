...
import { sourceLayoutMatchesV26Preset } from './FourCgsPresetExport';
...
describe('FourCgsPresetExport', () => {
  it('accepts the six quality-gated RAW4D sources in any picker order', () => {
    expect(sourceLayoutMatchesV26Preset([...sources].reverse())).toBe(true);
  });

  it('rejects a renamed, missing, or byte-different source', () => {
    expect(sourceLayoutMatchesV26Preset(sources.slice(1))).toBe(false);
    expect(sourceLayoutMatchesV26Preset(sources.map((source, index) => (
