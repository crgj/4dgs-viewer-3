import { decodeSharedShPartition, type PreparedSharedSh } from './FourCgsSharedSh';
/// <reference lib="webworker" />

import type { FourCgsManifest, FourCgsSegment } from './FourCgsTypes';
import { fourCgsDecodedPropertyNames } from './FourCgsRaw4D';

interface ShPartitionRequest {
  readonly prepared: PreparedSharedSh;
...
self.addEventListener('message', (event: MessageEvent<ShPartitionRequest>) => {
  const startedAt = performance.now();
  void import('../../../../../scripts/fourcgs-prs-codec.mjs').then(
    (codec) => {
      ...
      self.postMessage({ type: 'result', elapsedMs: performance.now() - startedAt });
    },
  ).catch(
